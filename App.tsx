...
import { 
  ...
  BookOpen, PlayCircle, Award, Flame, Link
} from 'lucide-react';
import { StudentProfile, CareerRecommendation, GroundingSource, ChatMessage, SkillSuggestion } from './types.ts';
import { getCareerRecommendations, searchMarketTrends, chatWithAI, getSkillSuggestions } from './services/geminiService.ts';

const INDIAN_LANGUAGES = [
  'English', 'Hindi (हिंदी)', 'Bengali (বাংলা)', 'Marathi (मराठी)', 
  'Telugu (తెలుగు)', 'Tamil (தமிழ்)', 'Gujarati (ગુજરાતી)', 'Urdu (اردو)', 
  'Kannada (ಕನ್ನಡ)', 'Odia (ଓڈિଆ)', 'Malayalam (മലയാളം)', 'Punjabi (ਪੰਜਾਬੀ)',
  'Assamese (অসমୀয়া)', 'Sanskrit (संस्कृतम्)'
];
...
const SmartBot: React.FC<{ language: string; t: any }> = ({ language, t }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      console.error("Pathweaver Bot Error:", e);
    } finally {
      setIsTyping(false);
    }
  };
...
const TrendsView: React.FC<{ language: string; t: any }> = ({ language, t }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-6 space-y-12 pb-24">
      ...
      {report && (
        <div className="glass rounded-[3rem] p-10 space-y-10 animate-in fade-in slide-in-from-bottom-8 shadow-xl">
          <div className="prose prose-slate dark:prose-invert max-w-none">
            {report.text.split('\n').map((para, i) => (
              para.trim() && <p key={i} className="mb-4 text-slate-700 dark:text-slate-300 leading-relaxed text-lg">{para}</p>
            ))}
          </div>

          {/* Grounding Sources: MANDATORY display */}
          {report.sources.length > 0 && (
            <div className="pt-8 border-t border-slate-100 dark:border-slate-800">
              <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-6 flex items-center gap-2">
                <Link className="w-3.5 h-3.5" /> {t.sources}
              </h4>
              <div className="grid sm:grid-cols-2 gap-4">
                {report.sources.map((src, i) => (
                  <a 
                    ...
                    className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 hover:border-indigo-400 transition-all group"
                  >
                    <span className="text-sm font-bold text-slate-700 dark:text-slate-300 truncate pr-4">{src.title}</span>
...
export default function App() {
  ...
  const handleGenerateRecs = async () => {
    ...
    try {
      ...
      alert("Something went wrong while generating your path. Please ensure your API key is configured and try again.");
    } finally {
